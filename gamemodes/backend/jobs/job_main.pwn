...
public Job_LoadData() {
    ...
    if(!rows) return print("[SQL]: Nije moguce ucitati tabelu `jobs`");
        ...
        for(new i = 0; i < rows; i++) {
            ...
            job_Label[i][0] = Create3DTextLabel(job_string, -1, job_Position[i][0], job_Position[i][1], job_Position[i][2], 3.5, 0, false);
            job_Label[i][1] = Create3DTextLabel(uniform_string, -1, job_UnifromPos[i][0], job_UnifromPos[i][1], job_UnifromPos[i][2], 3.5, job_Interior[i], false);
