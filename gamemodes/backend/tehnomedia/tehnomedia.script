...
public SQL_ElectronicLoad(playerid)
{
    ...
    if(!rows) 
    {
        ...
        mysql_format(SQL, q, sizeof(q), 
           "INSERT INTO `player_electronic` (player_id, Dron, Baterije, Navigacija) \ 
    ...
    else 
    {
        ...
        cache_get_value_name_int(0, "Baterije", PlayerElectronic[playerid][eBaterijaItem]);
        cache_get_value_name_int(0, "Navigacija", PlayerElectronic[playerid][eNavigacijaItem]);
    }
}
...
hook OnPlayerKeyStateChange(playerid, KEY:newkeys, KEY:oldkeys)
{
    if(newkeys & KEY_YES)
    {
        ...
        {
            Dialog_Show(playerid, "dialog_elektronik", DIALOG_STYLE_LIST,
...
Dialog: dialog_elektronik(const playerid, response, listitem, string: inputtext[])
{	
	...
	if(response)
	{
		switch(listitem)
		{
			...
			case 2:
			{
                if(PlayerElectronic[playerid][eBaterijaItem] == 2) return SendClientMessage(playerid, x_red, "tehnomedia » "c_white"Vec imate maksimalan broj baterija (2)!");
                ...
                VosticGiveMoney(playerid, -150);
                ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, false, false, false, false, 0);

                SendClientMessage(playerid, x_server, "tehnomedia » "c_white"Kupili ste baterije!");
			...
			case 3:
			{
                if(PlayerElectronic[playerid][eNavigacijaItem] == 1) return SendClientMessage(playerid, x_red, "tehnomedia » "c_white"Vec imate navigaciju!");
                if(GetPlayerMoney(playerid) < 300) return SendClientMessage(playerid, x_red, "tehnomedia » "c_white"Nemate dovoljno novca!");

                PlayerElectronic[playerid][eNavigacijaItem] = 1;

                VosticGiveMoney(playerid, -300);
                ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, false, false, false, false, 0);
