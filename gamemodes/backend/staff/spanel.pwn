	...
	if(response)
	{
		switch(listitem)
		{
			...
			case 3:
			{
				...
				ATMData[idzz][atmObjID] = Maryland_CreateDynamicObject(2942, ATMData[idzz][atmX], ATMData[idzz][atmY], ATMData[idzz][atmZ], ATMData[idzz][atmRX], ATMData[idzz][atmRY], ATMData[idzz][atmRZ]);
...
Dialog:dialog_odabirstanice(const playerid, response, listitem, string:inputtext[])
{
	if(!response) return 1;
	if(response)
	{
		new idmetroa;
		if(sscanf(inputtext, "i", idmetroa))
			return Dialog_Show(playerid, "dialog_odabirstanice", DIALOG_STYLE_INPUT, "Maryland - Metro Kreiraj:", "** Upisite zeljenu rutu za vas metro\n\nMetro stanice: 1 - Ruta1 | 1 - Ruta2 | 2 - Ruta2 | 3 - Ruta3 | 4 - Ruta4 | 5 - Ruta5", "Dalje", "Izlaz");


		new id = Iter_Free(Metros);

		if(id == -1)
			return SendClientMessage(playerid, 0xE74C3CFF, "Greska: {FFFFFF}Nema dovoljno slobodnih mesta za metro stanice");

		GetPlayerPos(playerid, metroInfo[id][MetroX], metroInfo[id][MetroY], metroInfo[id][MetroZ]);


		metroInfo[id][MetroRutaID] = idmetroa;
		...
		printf("%f, %f, %f",metroInfo[id][MetroX], metroInfo[id][MetroY], metroInfo[id][MetroZ]);
	}
	return (true);
}
...
Dialog:dialog_izmenistanicu(const playerid, response, listitem, string:inputtext[])
{
	if(!response) return 1;
	if(response)
	{
		...
		switch(listitem)
		{
			case 0: metroInfo[id][MetroRutaID] = 1;
			case 1: metroInfo[id][MetroRutaID] = 2;
			case 2: metroInfo[id][MetroRutaID] = 3;
			case 3: metroInfo[id][MetroRutaID] = 4;
			case 4: metroInfo[id][MetroRutaID] = 5;
		}
		va_SendClientMessage(playerid, 0xE74C3CFF, "Greska: {FFFFFF}Promenili ste rutu metroa id: %d",id);
	}
	return (true);
}
	...
	if(response)
	{
		switch(listitem)
		{
			...
			case 7:
			{
				new id = NearestMetro(playerid);
				if(id == -1)
					return SendClientMessage(playerid, x_server, "Niste u blizini metroa.");

				if(!Iter_Contains(Metros, id))
					return SendClientMessage(playerid, x_server, "Taj metro je vec obrisana.");

			    new szQuery[128];
			    format(szQuery, sizeof szQuery, "DELETE FROM `metros` WHERE `metroID`='%d'", id);
			    mysql_tquery(SQL, szQuery);
...
Dialog:dialog_kreirajkucu(const playerid, response, listitem, string:inputtext[]) {
	...
	print("kreirajkucu dialog");
	if(response) {

		new id = Iter_Free(iHouse), Float:pPos[3];
		GetPlayerPos(playerid, pPos[0], pPos[1], pPos[2]);

		print("kreirajkucu dialog prosao");
		switch(listitem) {

			case 0: {


				print("dialog_kreirajkucu case 0");

				house_Price[id] = 50000;
				house_Type[id] = 1;
			...
			case 1: {
				...
				print("dialog_kreirajkucu case 1");
			...
			case 2: {
				...
				print("dialog_kreirajkucu case 2");
				house_Price[id] = 300000;
				house_Type[id] = 3;
				...
				house_Exit[id][0] = 2324.38; 
				house_Exit[id][1] = -1148.48; 
				house_Exit[id][2] = 1050.71;
		...
		print("dialog_kreirajkucu prosao case");
		new zone[28], add[70];
		GetPlayer2DZone(playerid, zone, sizeof(zone));
		format(add, sizeof(add), "%s, Maryland", zone);
		strmid(house_Adress[id], add, 0, sizeof(add), 255);
		...
		Iter_Add(iHouse, id);
		print("dolazi do kverija");

		new query[900];
		mysql_format(SQL, query, sizeof query, "INSERT INTO `houses` (`PID`, `Price`, `Type`, `Adress`, `Locked`, `PosX`,`PosY`, `PosZ`, `ExitX`, `ExitY`, `ExitZ`, `Safe`, `Money`,`Weed`, `Cocaine`, `Extazy`, `WardX`, `WardY`, `WardZ`,`FridgeX`, `FridgeY`, `FridgeZ`, `Int`) \
